...
const PatientIntakeForm: React.FC<PatientIntakeFormProps> = ({ patientData, onDataChange, onSubmit, onStartVoiceIntake, onStartImageAnalysis }) => {
  ...
  return (
    <div className="animate-fade-in h-full flex flex-col">
        <p className="text-center text-slate-400 text-sm mb-4">Please provide your details manually, or use one of our AI-powered intake options below.</p>
        <form onSubmit={handleSubmit} className="space-y-3 flex-grow overflow-y-auto pr-2">
                 ...
                 <div className="space-y-3">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                        <div>
                            ...
                            <input type="text" name="fullName" id="fullName" value={patientData.fullName} onChange={handleChange} required className={inputStyle} placeholder="e.g., Jane Doe" />
                        </div>
                        <div>
                    ...
                    <div>
                        ...
                        <input type="text" name="contact" id="contact" value={patientData.contact} onChange={handleChange} required className={inputStyle} placeholder="email@example.com" />
                    </div>
                    <div>
                        <label htmlFor="symptoms" className={labelStyle}>Please describe your symptoms</label>
                        <textarea name="symptoms" id="symptoms" rows={2} value={patientData.symptoms} onChange={handleChange} required className={inputStyle} placeholder="e.g., Sharp chest pains..."></textarea>
                    </div>
                </div>
            </fieldset>
            ...
            <fieldset className="!mt-4">
                 ...
                 <div className="space-y-3">
                    <div>
                        ...
                        <textarea name="pastConditions" id="pastConditions" rows={2} value={patientData.pastConditions} onChange={handleChange} className={inputStyle} placeholder="e.g., Asthma, Hypertension..."></textarea>
                    </div>
                    <div>
                        <label htmlFor="surgeries" className={labelStyle}>Previous Surgeries</label>
                        <textarea name="surgeries" id="surgeries" rows={2} value={patientData.surgeries} onChange={handleChange} className={inputStyle} placeholder="e.g., Appendectomy (2015)..."></textarea>
                    </div>
                     <div>
                        <label htmlFor="medications" className={labelStyle}>Current Medications</label>
                        <textarea name="medications" id="medications" rows={2} value={patientData.medications} onChange={handleChange} className={inputStyle} placeholder="e.g., Lisinopril 10mg..."></textarea>
                    </div>
                </div>
            </fieldset>
         ...
         <div className="mt-4 relative flex-shrink-0">
            ...
            <div className="relative flex justify-center">
                <span className="bg-black/20 px-2 text-sm text-slate-500 backdrop-blur-sm">Or use an AI Intake Method</span>
            </div>
        </div>
        <div className="mt-4 grid grid-cols-2 gap-3 flex-shrink-0">
            ...
            <button
                ...
                className="w-full flex justify-center items-center py-2 px-4 border border-slate-600 rounded-md shadow-sm text-sm font-medium text-slate-300 bg-slate-500/10 hover:bg-slate-500/20 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-slate-900 focus:ring-cyan-500 transition-colors"
            >
                <DocumentScannerIcon className="w-5 h-5 mr-2 text-slate-400" />
